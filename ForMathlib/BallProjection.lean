...
/-!
# The radial retraction onto the closed unit ball

For a real inner product space `E`, `ballProj x = (‖x‖ ⊔ 1)⁻¹ • x` is the metric projection of `x`
onto the closed unit ball `{y | ‖y‖ ≤ 1}`: the identity inside the ball, and the radial retraction
`x / ‖x‖` outside it. Mathlib has the orthogonal projection onto a *subspace*
(`Submodule.lipschitzWith_orthogonalProjection`) but not the metric projection onto the closed unit
ball, so this file proves its defining variational inequality and its nonexpansiveness
(`1`-Lipschitz) from scratch, via firm nonexpansiveness and Cauchy-Schwarz.
...
/-- **The radial retraction is `1`-Lipschitz** (nonexpansive), the metric-projection property.
Proved by firm nonexpansiveness: `‖ballProj x - ballProj y‖² ≤ ⟪x - y, ballProj x - ballProj y⟫`,
from the two variational inequalities, then Cauchy–Schwarz. -/
