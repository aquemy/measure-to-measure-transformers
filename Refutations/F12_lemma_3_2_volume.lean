...
-- family form since the Section-3.3 restatement). Derives the kernel-refuted pre-F12 statement
-- (every measure, no probability/sphere/cap hypotheses) from the current family axiom by
-- instantiating a one-member family. If this file ever COMPILES, `lemma_3_2` has been
...
example : Regression.OldLemma32Sig :=
  fun μ T hT =>
    ((lemma_3_2 (fun _ : Fin 1 => μ) (fun _ => inferInstance) T hT).imp
      fun θ h => h.2)
