...
![The de-axiomatization build DAG, in the house sketch style. M8a (green) was the standalone first target; the linchpin M3 (gold), M2, M4, and M5 are all discharged, and the mean-field layer M3b sits between M3 and the measure-dependent milestones. (The node colours show the original plan; the live per-node status is on the [claim graph](claimgraph.qmd).)](diagrams/build-dag.ink.svg){width=100%}
