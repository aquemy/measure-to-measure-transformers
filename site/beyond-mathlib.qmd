...
- **M5 -- Geodesic convexity on the sphere: [in progress]{.badge .partial}.** The geometric foundation
  ...
  geometry that consumes these facts; the paper axiom `exists_disentangling_balls` (now stated on the
  mean-field layer, M3b) waits on that geometry *and* on mean-field dynamics facts, so it stays
  `axiomatised` for now.
