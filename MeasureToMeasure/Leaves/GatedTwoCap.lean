...
/-- The logistic coordinate stays in `(-1, 1)` along the scaled flow (poles avoided): the flow from
`x ≠ ±ω` never reaches `±ω` (fixed points of the scaled field, so reaching one would force `x` to be
it by injectivity). -/
theorem inner_scaledFlow_mem_Ioo {A : ℝ} {z ω : Eucl d} (hω : ω ∈ sphere d) (cosR : ℝ)
    (b : Block d) (hfield : b.field = scaledGatedField A z ω cosR)
    {x : Eucl d} (hx : x ∈ sphere d) (hne : x ≠ ω) (hne' : x ≠ -ω) {t : ℝ} (ht : 0 ≤ t) :
    (⟪b.blockFlow t x, ω⟫ : ℝ) ∈ Set.Ioo (-1 : ℝ) 1 := by
  have hnp : b.blockFlow t x ≠ ω := fun hcontra =>
    hne (b.blockFlow_injective t (hcontra.trans
      (b.blockFlow_fixed (by rw [hfield]; exact scaledGatedField_pole_eq_zero hω cosR) t).symm))
  have hnnp : b.blockFlow t x ≠ -ω := fun hcontra =>
    hne' (b.blockFlow_injective t (hcontra.trans
      (b.blockFlow_fixed (by rw [hfield]; exact scaledGatedField_neg_pole_eq_zero hω cosR) t).symm))
  exact inner_mem_Ioo_of_ne (b.blockFlow_mem_sphere hx ht) hω hnp hnnp
